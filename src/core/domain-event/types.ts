...
export type DomainEventConstructor<Name extends ValueObjectFQN<string, `DomainEvent::${string}`> = any, Payload = any> = Compute<
  {
    from(p: Payload): ValueObject<ValueObjectFQN<string, `DomainEvent::${string}::Payload`>, { payload: Payload }>;
